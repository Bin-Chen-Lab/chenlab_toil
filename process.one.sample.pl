...
use strict;
my $srr_id               = $ARGV[0];
my $STAR_INDEX           = "path of STAR index"; 
my $RSEM_INDEX           = "path of hg38.RSEM index";
#my $KALL_INDEX          = "path of KALLISTO/hg38.transcript.index";
...
#if(! -e "KALLISTO.output/$srr_id"){#`$kall_cmd`; ok, currently we do not care about kallisto}
